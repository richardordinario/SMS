...
$request = $_REQUEST;
$col = array("subject_code","subject_desc");
...
if(!empty($request['search']['value'])) {
    $sql .= " AND (subject_code LIKE '".$request['search']['value']."%' ";
    $sql .=  "OR subject_desc LIKE '".$request['search']['value']."%' )";
}
...
while($row = mysqli_fetch_array($query)) {
	...
	$subdata[] = '<span style="font-size: 12px;font-weight: 500;">'.$row["subject_desc"].'</span>';
