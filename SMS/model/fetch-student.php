...
$request = $_REQUEST;
$col = array("studid","fname","lname","mname","course","ylevel","section");
...
if(!empty($request['search']['value'])) {
    $sql .= " AND (studid LIKE '".$request['search']['value']."%' ";
...
while($row = mysqli_fetch_array($query)) {
	$subdata = array();
	$subdata[] = '<span style="font-size: 12px;font-weight: 600;">'.$row["studid"].'</span>';
    ...
    $subdata[] = '<span style="font-size: 11px;font-weight: 600;">'.$row["course"].'</span>';
    $subdata[] = '<span style="font-size: 12px;font-weight: 500;">'.$row["ylevel"].'</span>';
	$subdata[] = '<span style="font-size: 12px;font-weight: 600;">'.$row["section"].'</span>';
